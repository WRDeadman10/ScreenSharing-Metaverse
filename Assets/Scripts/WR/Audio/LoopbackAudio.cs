...
public class LoopbackAudio : MonoBehaviour
{
    ...
    private void AuidioRecierdBytes(float[] obj)
    {
        if (VoiceChatManager.Instance == null)
            return;
        if (VoiceChatManager.Instance.inCall)
        {
            if (VoiceChatManager.Instance.shareScreen)
            {
                //VoiceChatManager.Instance.PushExternalAudioFrame(obj, 1);
            }
        }
    }
    public void Update()
